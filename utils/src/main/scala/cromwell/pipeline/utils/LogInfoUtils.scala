...
object LogInfoUtils {
  ...
  def getConfigMessage(applicationConfig: ApplicationConfig): String = {
    ...
    implicit val wsWrites: Writes[WebServiceConfig] = ((__ \ "interface").write[String] ~
      (__ \ "port").write[Int])(unlift(WebServiceConfig.unapply))

    implicit val expTimeWrites: Writes[ExpirationTimeInSeconds] = ((__ \ "accessToken").write[Long] ~
      (__ \ "refreshToken").write[Long] ~
      (__ \ "userSession").write[Long])(
      unlift(ExpirationTimeInSeconds.unapply)
    )

    implicit val authWrites: Writes[AuthConfig] = ((__ \ "secretKey").write[String] ~
    ...
    implicit val gitLabWrites: Writes[GitLabConfig] = ((__ \ "url").write[String] ~
    ...
    implicit val mongoWrites: Writes[MongoConfig] = ((__ \ "user").write[String] ~
      ...
      mongoConfig => unlift(MongoConfig.unapply)(mongoConfig).copy(_2 = mongoConfig.password.map(_ => "*").mkString(""))
    )

    implicit val postgreWrites: Writes[PostgreConfig] = ((__ \ "serverName").write[String] ~
    ...
    implicit val appConfigWrites: Writes[ApplicationConfig] = ((__ \ "WebServiceConfig").write[WebServiceConfig] ~
